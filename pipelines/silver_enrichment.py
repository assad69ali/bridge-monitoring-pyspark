...
def main(run_seconds: int = 30):
    ...
    Runs structured streaming for a limited time (default 30 seconds),
    then stops all queries and shuts down Spark.
    """
    spark = (
    ...
    # metadata/bridge_metadata.csv must exist
    bridges_df = (
        spark.read
        .option("header", "true")
        .schema(bridges_schema)
        .csv("metadata/bridge_metadata.csv")
    )
    ...
    # ---- EXTRA: console view of enriched data ----
    temp_tagged = temp_enriched.withColumn("source", lit("temperature"))
    vib_tagged  = vib_enriched.withColumn("source", lit("vibration"))
    tilt_tagged = tilt_enriched.withColumn("source", lit("tilt"))

    all_enriched = (
        temp_tagged
        .unionByName(vib_tagged, allowMissingColumns=True)
        .unionByName(tilt_tagged, allowMissingColumns=True)
    )

    q_console = (
        all_enriched.writeStream
        .format("console")
        .outputMode("append")
    ...
    # ---- time-bounded run ----
    print(f"Silver streaming started, will run for {run_seconds} seconds...")
    start = time.time()
    ...
    try:
        ...
        spark.stop()
        print("Silver pipeline finished.")
